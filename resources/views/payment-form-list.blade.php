            ...
            <?php $total = 0; ?>
              @if(!empty($receipt_forms))
                @foreach($receipt_forms as $rf)
                    <?php 
                      $total += $rf->amount; 
                      $fromledgers = explode(',', $rf->fromLedger->name);
                    ...
                    <tr>
                        <td>
                          @foreach($fromledgers as $k => $fromledger)
                            <p>
                              @if(!empty($rf->fromLedger->wing_flat_no) && $k == 0)
                                {{$rf->fromledger->wing_flat_no}} -
                              @endif
                              {{$fromledger}}
                              @if($cfromledger > $k+1)
                                &
                        ...
                        <td> 
                            ...
                            <a href="{{url('payment-voucher/edit-form/'.$rf->id)}}" class="btn btn-primary btn-icon pt-1">
                              <i data-feather="edit"></i>
                            </a>
                            <a href="{{url('payment-voucher/delete-form/'.$rf->id)}}" class="btn btn-danger btn-icon pt-1">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Payment Accept</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form class="forms-sample" action="{{route('payment-voucher.payment-form-accept')}}" method="POST" enctype="multipart/form-data">
                    @csrf
