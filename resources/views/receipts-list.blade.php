...
<body data-base-url="{{url('/')}}">
  ...
  <div id="app">
      ...
      <div class="page-content">
            <div class="row">
                <div class="col-md-12 grid-margin stretch-card">
                    <div class="card">
                        @if(!empty($ledger) && !empty($receipts_voucher))
                            <div class="card-body">        
                                <h6 class="card-title">
                                    ...
                                    @if(!empty($receipts_voucher))
                                        @foreach($receipts_voucher as $rv)
                                            ...
                                            <tr>
                                                ...
                                                <td>  
                                                <a href="{{url('search-receipts/download/'.$rv->id)}}" class="btn btn-info btn-icon pt-1">
                                                    <i data-feather="download"></i>
                                                </a>  
                                                <a href="{{url('search-receipts/view/'.$rv->id)}}" class="btn btn-success btn-icon pt-1">
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        ...
                                        {{ $receipts_voucher->links('pagination::bootstrap-4') }}
                                        </div>
                                    </div>
                                </div>
                                </div>
                        ...
                        @else
                            <div class="card-body">        
                                <h6 class="card-title">
                                    @if(Session::get('receipts_ledger_name') && Session::get('receipts_wing_flat_no'))
                                        {{Session::get('receipts_wing_flat_no')}} - {{Session::get('receipts_ledger_name')}}
                                    @endif
                                    <a type="button" href="{{ route('search-receipts-create-zip', [0, 0]) }}" class="btn float-right btn-outline-secondary btn-icon-text mr-2 d-none d-md-block">
    ...
    <script>
        ...
        {
            var url = '{{config("app.url")}}'+'/search-receipts/send/'+id;
            $.ajax({
               type:'GET',
               url:url,
