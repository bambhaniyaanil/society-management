    ...
    <div class="card">
      <div class="card-body">
        <h6 class="card-title">{{$page_title}}</h6>
            ...
            @endif 
          <form method="POST" action="{{url('role-permission/add')}}" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="row">
                <div class="col-sm-6">
                ...
                @foreach($pmenus as $pmenu)
                    <?php $smenu = App\Menus::where('pmenu', $pmenu->mid)->where('mid', '!=', $pmenu->mid)->get(); ?>
                    <div class="col-sm-12">
                        <div class="form-group">
                            <div class="form-check">
                                <label class="form-check-label">
                                    <input type="checkbox" name="pid[]" class="form-check-input" value="{{$pmenu->mid}}">
                                        {{$pmenu->mname}}
                                </label>
                            </div>
                        </div>
                                ...
                                @foreach($smenu as $sm)
                                    <div class="col-sm-3">
                                        <div class="form-group">
                                            <div class="form-check">
                                                <label class="form-check-label">
                                                    <input type="checkbox" name="pid[]" class="form-check-input" value="{{$sm->mid}}">
                                                        {{$sm->mname}}
                                                </label>
                                            </div>
                                        </div>
